...
function getScales(data: Data[], chartWidth: number, chartHeight: number) {
    ...
    return {
        xScale: d3.scaleBand().domain(data.map(d => d.label)).range([0, chartWidth]).padding(0.2),
        yScale: d3.scaleLinear().domain([0, d3.max(data, d => d.value)! < 1 ? 1 : d3.max(data, d => d.value) as number]).range([chartHeight, 0]).nice(),
    }
}
    ...
    React.useEffect(() => {
            if (d3Container.current) {
                ...
                const bars = barsGroup
                    .selectAll<SVGGElement, Data>('rect')
                    .data(props.data)
